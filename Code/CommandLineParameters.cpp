...
namespace FileReadSpeedTest {
	...
	Action ProcessCommandLineParameters(int argc, const char * argv[]) noexcept {
		...
		auto state_machine = max::Containers::StateMachine::StateMachine{NodeIndices::StartNode, std::make_tuple(start_node, awaiting_flags_node, std::move(awaiting_input_file_node), std::move(error_node), awaiting_buffer_size_node)};
		...
		return Action{SuccessAction{std::move(*input_file), std::move(buffer_size)}};
	}
